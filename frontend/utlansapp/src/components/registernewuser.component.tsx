...
const RegisternewuserComponent = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidName(fornavn)) {
    ...
    } else if (!isValidClass(klasse)) {
      console.log("klasse er ikke riktig format!");
      ...
      axios
        .post("http://localhost:4000/brukere/register", {
          fornavn: fornavn,
          etternavn: etternavn,
          epost: mail,
          telefonnummer: tlf,
    ...
    <form onSubmit={handleSubmit}>
      ...
      <label>
        ...
        <input
          type="text"
          value={mail}
          placeholder="Skriv inn din osloskolemail"
      ...
      <label>
        ...
        <input
          type="number"
          value={tlf}
          placeholder="Skriv inn ditt tlfnr"
          onChange={(event) => setTlf(parseInt(event.target.value))}
        />
      </label>
      <br />
      <label>
      ...
      <label>
        ...
        <input
          type="text"
          value={kontaktlarer}
          placeholder="Fullt navn på din kontaktlærer"
          onChange={(event) => setKontaktlarer(event.target.value)}
        />
      </label>
      <br />
      <label>
